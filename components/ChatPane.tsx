  ...
  return (
    <div className="flex h-full flex-col bg-white dark:bg-neutral-900">
      ...
      <div className="flex shrink-0 items-end gap-2 border-t border-neutral-200 p-2 dark:border-neutral-800">
        ...
        <button onClick={handleSend} disabled={sending || (!autoMode && !selectedModel)} className="flex shrink-0 items-center gap-1.5 rounded bg-blue-600 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-blue-500 disabled:opacity-50">{sending && <Spinner size={10} />}{sending ? "Sending" : "Send"}</button>
      </div>
    </div>
  );
}