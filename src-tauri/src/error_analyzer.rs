use crate::planning_engine::{Subtask, TaskPlan};
    ...
    pub fn is_recoverable(&self) -> bool {
        !matches!(
            ...
            FailureCategory::NetworkError
                ...
                | FailureCategory::FileSystemError
        )
    }
...
impl ErrorAnalyzer {
    pub fn analyze(exec_result: &ExecutionResult) -> FailureReport {
        ...
        if exec_result.exit_code != 0 && failures.is_empty() {
            failures.push(DiagnosticFailure {
                ...
                raw_message: format!(
                    "Process exited with code {} but no diagnostics were parsed from output",
                    exec_result.exit_code
                ),
                suggested_fix: "Review stdout/stderr manually to identify the failure cause"
                    .to_string(),
            });
        }
        let retry_suggested =
            !failures.is_empty() && failures.iter().any(|f| f.category.is_recoverable());
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            if trimmed.contains("permission denied") || trimmed.contains("Permission denied") {
                failures.push(DiagnosticFailure {
                    ...
                    suggested_fix: "Check file permissions and ensure the process has write access"
                        .to_string(),
                });
                continue;
            }
            if let Some(rest) = trimmed.strip_prefix("error[") {
                if let Some(code_end) = rest.find(']') {
                    ...
                    let msg = rest[code_end + 1..]
                        .trim()
                        .trim_start_matches(':')
                        .trim()
                        .to_string();
                    let (file_name, line_num, col_num) =
                        if lines.get(i + 1).map_or(false, |n| n.contains("-->")) {
                            Self::extract_location(lines[i + 1].trim())
                        } else {
                            Self::extract_location(trimmed)
                        };
                    let category = if ec.starts_with("E06")
                        || ec.starts_with("E05")
                        || ec.starts_with("E03")
                    {
                        FailureCategory::TypeCheckError
                    } else if ec.starts_with("E04") {
            ...
            if trimmed.starts_with("error:") || trimmed.starts_with("error :") {
                ...
                failures.push(DiagnosticFailure {
                    ...
                    error_code: None,
                    raw_message: msg,
                    suggested_fix: "Review the error message and fix the compilation issue"
                        .to_string(),
                });
                continue;
            }
            ...
            if let Some(rest) = trimmed.strip_prefix("error TS") {
                ...
                if let Some(ce) = rest.find(':') {
                    ...
                    let file = rest.find(':').and_then(|ci| {
                        let fp = &rest[..ci];
                        if fp.contains('(') {
                            Some(fp.split('(').next().unwrap_or("").to_string())
                        } else {
                            None
                        }
                    });
    ...
    fn extract_location(line: &str) -> (Option<String>, Option<usize>, Option<usize>) {
        for chunk in line.split(|c: char| c == ' ' || c == ':') {
            if chunk.contains(".rs")
                || chunk.contains(".ts")
                || chunk.contains(".tsx")
                || chunk.contains(".js")
                || chunk.contains(".py")
            {
                let f = if chunk.contains('(') {
    ...
    fn suggest_fix(code: &str, msg: &str) -> String {
        if code == "E0432" || code == "E0433" {
            "Add the missing `use` import statement".into()
        } else if code == "E0609" {
            "The field or method does not exist — check the type definition".into()
        } else if msg.contains("cannot find") {
            "Ensure the identifier is in scope or imported correctly".into()
        } else {
            "Review the error and fix the compilation issue".into()
        }
    }
}
...
impl RetryState {
    ...
    pub fn next_delay_ms(&self) -> u64 {
        (1000u64 * 2u64.pow(self.attempt_count)).min(30000)
    }
...
impl RepairContextBuilder {
    pub fn build_repair_plan(
        ...
        let affected_files: Vec<String> = report
            ...
            .collect();
        let combined = if affected_files.is_empty() {
        ...
        TaskPlan {
            ...
            subtasks: if subtasks.is_empty() {
                vec![Subtask {
                    id: 0,
                    description: "Review execution output manually".into(),
                    dependencies: vec![],
                    required_files: original_plan.affected_files.clone(),
                    expected_outcome: "All errors resolved".into(),
...
mod tests {
    ...
    #[test]
    fn parses_cargo_error() {
        let r = mr("","error[E0308]: mismatched types\n  --> src/main.rs:10:5\n10 |     let x: i32 = \"hello\";\n   |                ^^^^^^^ expected i32",101);
        let rep = ErrorAnalyzer::analyze(&r);
        assert!(!rep.failures.is_empty());
        assert!(rep
            .failures
            .iter()
            .any(|f| f.category == FailureCategory::TypeCheckError));
    }
    #[test]
    fn parses_test_failure() {
        ...
        assert!(ErrorAnalyzer::analyze(&r)
            .failures
            .iter()
    ...
    fn parses_timeout() {
        ...
        assert!(ErrorAnalyzer::analyze(&r)
            .failures
            .iter()
            .any(|f| f.category == FailureCategory::Timeout));
    }
    #[test]
    fn retry_respects_limits() {
        let mut s = RetryState::new("t1", 3);
        s.record_failure(&FailureReport {
            failures: vec![],
            analysis_summary: "x".into(),
    ...
    fn backoff_grows() {
        ...
        let mut s2 = s.clone();
        s2.attempt_count = 2;
        assert!(s2.next_delay_ms() > s.next_delay_ms());
    }
    #[test]
    fn repair_plan_files() {
        let r = mr(
            "",
            "error[E0609]: no field `foo`\n  --> src/config.rs:42:10",
            101,
        );
        let orig = TaskPlan {
        ...
        let state = RetryState::new("t1", 3);
        let plan = RepairContextBuilder::build_repair_plan(&orig, &r, &state);
        assert!(plan
            .subtasks
            .iter()
            .any(|s| s.description.contains("config.rs")));
    }
    #[test]
    fn non_recoverable() {
        let r = mr("", "error: could not write to disk — permission denied", 1);
        let rep = ErrorAnalyzer::analyze(&r);
        assert!(!rep.failures.iter().any(|f| f.category.is_recoverable()));
    }
    #[test]
    fn missing_dep() {
        let r = mr("", "error TS2307: Cannot find module './missing'", 2);
        assert!(ErrorAnalyzer::analyze(&r)
            .failures
            .iter()
            .any(|f| f.category == FailureCategory::MissingDependency));
    }
    #[test]
    fn empty_ok() {
        assert!(ErrorAnalyzer::analyze(&mr("Compiled", "", 0))
            .failures
            .is_empty());
    }
}
