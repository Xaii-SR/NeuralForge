...
use crate::context_retrieval::{ContextRetrieval, RankedFile};
...
impl TaskOrchestrator {
    ...
    pub fn plan_with_db(
        ...
        let ctx = task
            ...
            .as_mut()
            .ok_or_else(|| "No agent context".to_string())?;
        let dep_graph = ContextRetrieval::build_repository_graph(conn).unwrap_or_default();
    ...
    pub fn recover(
        ...
        for _ in 0..task.recovery_attempts {
            retry_state.record_failure(&FailureReport {
                failures: vec![],
                analysis_summary: "previous".into(),
        ...
        let repair = crate::error_analyzer::RepairContextBuilder::build_repair_plan(
            original,
            exec_result,
            &retry_state,
        );
        task.current_plan = Some(repair.clone());
        let steps: Vec<String> = repair
            .subtasks
            .iter()
    ...
    pub fn verify(task: &mut OrchestratorTask, exec_result: &ExecutionResult) -> bool {
        ...
        if passed {
            ...
            Self::transition(
                task,
                TaskLifecycle::Failed(format!("Verification failed: {}", report.analysis_summary)),
            );
        }
        passed
    }
    ...
    fn default_completion_criteria() -> Vec<String> {
        vec![
            "Workspace analysis completed".into(),
            "Plan generated".into(),
            "All steps executed".into(),
...
pub fn orchestrator_create_task(
    ...
    let workspace_root = app_state
        ...
        .clone()
        .ok_or_else(|| {
            "no workspace open - cannot start a task without a real workspace root".to_string()
        })?;
...
#[tauri::command]
pub fn orchestrator_approve_task(
    app: AppHandle,
    state: State<'_, OrchestratorState>,
) -> Result<OrchestratorTask, String> {
...
#[tauri::command]
pub fn orchestrator_reject_task(
    app: AppHandle,
    state: State<'_, OrchestratorState>,
) -> Result<OrchestratorTask, String> {
    let mut guard = state.active_task.lock().unwrap();
...
#[tauri::command]
pub fn orchestrator_cancel_task(
    app: AppHandle,
    state: State<'_, OrchestratorState>,
) -> Result<(), String> {
    let mut guard = state.active_task.lock().unwrap();
...
pub fn orchestrator_get_state(
    ...
    state
        ...
        .as_ref()
        .cloned()
        .ok_or("No active task".into())
}

#[tauri::command]
pub fn orchestrator_reset(
    app: AppHandle,
    state: State<'_, OrchestratorState>,
) -> Result<(), String> {
...
mod tests {
    ...
    #[test]
    fn task_creation() {
        let t = TaskOrchestrator::create_task("Fix auth bug", PathBuf::from("/tmp"));
        assert_eq!(t.phase, TaskLifecycle::Created);
    ...
    #[test]
    fn create_task_preserves_the_given_workspace_root_verbatim() {
        let root = PathBuf::from("/some/real/workspace");
        let t = TaskOrchestrator::create_task("goal", root.clone());
        assert_eq!(
            t.workspace_root, root,
            "must propagate the real workspace root, not silently substitute \".\""
        );
    }
    #[test]
    fn state_transitions() {
        let mut t = TaskOrchestrator::create_task("Test", PathBuf::from("/tmp"));
        TaskOrchestrator::transition(&mut t, TaskLifecycle::Analyzing);
        assert_eq!(t.phase, TaskLifecycle::Analyzing);
        TaskOrchestrator::transition(&mut t, TaskLifecycle::Planning);
        assert!(t.execution_history.len() >= 2);
    }
    #[test]
    fn analyze_and_plan() {
        let mut t = TaskOrchestrator::create_task("Add logging", PathBuf::from("/tmp"));
        ...
        TaskOrchestrator::analyze(&mut t, ranked);
        assert_eq!(t.phase, TaskLifecycle::Planning);
        assert!(t.agent_context.is_some());
        assert!(TaskOrchestrator::plan(&mut t).is_ok());
    }
    #[test]
    fn execute_and_observe() {
        ...
        TaskOrchestrator::analyze(&mut t, ranked);
        TaskOrchestrator::plan(&mut t).unwrap();
        TaskOrchestrator::execute_step(&mut t, 0, &PathBuf::from("/tmp")).unwrap();
        ...
        TaskOrchestrator::observe(&mut t, &r);
        assert_eq!(t.phase, TaskLifecycle::Verifying);
    }
    #[test]
    fn recovery_on_failure() {
        let mut t = TaskOrchestrator::create_task("fail", PathBuf::from("/tmp"));
        let ranked = vec![RankedFile {
            path: "a.rs".into(),
            language: "Rust".into(),
            priority: 100,
        ...
        TaskOrchestrator::observe(&mut t, &r);
        assert!(matches!(t.phase, TaskLifecycle::Recovering { .. }));
    }
    #[test]
    fn max_recovery_fails() {
        let mut t = TaskOrchestrator::create_task("fail", PathBuf::from("/tmp"));
        t.max_recovery_attempts = 0;
        let ranked = vec![RankedFile {
            path: "a.rs".into(),
            language: "Rust".into(),
            priority: 100,
        ...
        TaskOrchestrator::observe(&mut t, &r);
        assert!(matches!(t.phase, TaskLifecycle::Failed(_)));
    }
    #[test]
    fn cancels() {
        let mut t = TaskOrchestrator::create_task("cancel", PathBuf::from("/tmp"));
        TaskOrchestrator::transition(&mut t, TaskLifecycle::Analyzing);
        TaskOrchestrator::cancel(&mut t);
        assert_eq!(t.phase, TaskLifecycle::Cancelled);
    }
    #[test]
    fn progress() {
        let mut t = TaskOrchestrator::create_task("p", PathBuf::from("/tmp"));
        assert_eq!(t.progress_percent(), 0.0);
        TaskOrchestrator::transition(&mut t, TaskLifecycle::Completed);
        assert_eq!(t.progress_percent(), 100.0);
    }
    #[test]
    fn persistence() {
        let t = TaskOrchestrator::create_task("persist", PathBuf::from("/tmp"));
        let s = serde_json::to_string(&t).unwrap();
        let r: OrchestratorTask = serde_json::from_str(&s).unwrap();
        assert_eq!(r.id, t.id);
    }
}
