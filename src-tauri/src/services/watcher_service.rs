...
pub fn apply_events(
    ...
    for event in events {
        ...
        for path in paths {
            let Some(rel_path) = visit(&path) else {
                continue;
            };
...
impl WorkspaceWatcher {
    pub fn start(
        ...
        .map_err(|e| {
            crate::core::errors::AppError::Provider(format!("failed to start file watcher: {e}"))
        })?;
        watcher
            .watch(&workspace_root, RecursiveMode::Recursive)
            .map_err(|e| {
                crate::core::errors::AppError::Provider(format!(
                    "failed to watch workspace root: {e}"
                ))
            })?;
                ...
                if ready {
                    ...
                    match crate::database::open_for_workspace(&thread_root) {
                        Ok(conn) => match apply_events(&conn, &thread_root, &batch) {
                            ...
                            Ok(_) => {}
                            Err(error) => {
                                tracing::warn!(target: "watcher", event = "apply_events_failed", error = %error)
                            }
                        },
                        Err(error) => {
                            tracing::warn!(target: "watcher", event = "watcher_connection_failed", error = %error)
                        }
                    }
                }
            }
...
impl WatcherService {
    pub fn new(debounce_ms: u64) -> Self {
        Self {
            debouncer: Arc::new(Mutex::new(DebouncedEvents::new(debounce_ms))),
        }
    }
...
mod tests {
    ...
    fn apply_events_indexes_a_created_file() {
        ...
        let count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM files WHERE path = 'new.rs'",
                [],
                |r| r.get(0),
            )
            .unwrap();
    ...
    fn apply_events_reindexes_a_modified_file() {
        ...
        let symbol_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM symbols WHERE file_path = 'lib.rs'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(
            symbol_count, 2,
            "the modified content must be fully reindexed, not left stale"
        );
    ...
    fn apply_events_purges_a_removed_file() {
        ...
        let before: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM files WHERE path = 'gone.rs'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        ...
        assert_eq!(
            after, 0,
            "a deleted file must be purged from the index, not left as a stale row"
        );
    ...
    fn apply_events_handles_rename_as_purge_old_index_new() {
        ...
        let old_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM files WHERE path = 'old_name.rs'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        let new_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM files WHERE path = 'new_name.rs'",
                [],
                |r| r.get(0),
            )
            .unwrap();
    ...
    fn apply_events_respects_the_exclusion_policy() {
        ...
        assert_eq!(
            count, 0,
            "excluded paths must never enter the index via watcher events either"
        );
    ...
    fn apply_events_ignores_paths_outside_the_workspace_root() {
        ...
        assert!(
            result.is_ok(),
            "a path outside the workspace root must be skipped, not error the whole batch"
        );
        let count: i64 = conn
            .query_row("SELECT COUNT(*) FROM files", [], |r| r.get(0))
            .unwrap();
    ...
    fn workspace_watcher_indexes_a_real_file_creation_end_to_end() {
        ...
        assert!(
            captured
                .iter()
                .any(|e| matches!(e, FileEvent::Created(p) if p == &new_file)),
        ...
        let count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM files WHERE path = 'created_by_watcher.rs'",
                [],
                |r| r.get(0),
            )
            .unwrap();
