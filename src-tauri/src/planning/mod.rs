...
pub fn plan_requirement_dag(
    ...
    let root = state
        ...
        .clone()
        .ok_or_else(|| {
            crate::core::errors::AppError::InvalidPath("no workspace open".to_string())
        })?;
    ...
    for spec in &specs {
        ...
        if !canonical_target.starts_with(&canonical_root) {
            return Err(crate::core::errors::AppError::InvalidPath(format!(
...
pub fn get_dag(db: State<DbState>, dagId: String) -> AppResult<planner::TaskDagRecord> {
    with_conn(&db, |conn| {
        planner::load_dag(conn, &dagId).map(|(record, _)| record)
    })
}

#[tauri::command]
pub fn get_dag_runnable_tasks(
    db: State<DbState>,
    dagId: String,
...
#[cfg(test)]
mod tests {
    use super::planner::{load_dag, plan_dag, DagTaskSpec};
    ...
    ) -> crate::governance::requirements::RequirementContract {
    ...
    fn cycle_is_rejected_before_any_row_is_written() {
        ...
        let result = plan_dag(
            &conn,
            &req,
            &[spec("a.md", &[1]), spec("b.md", &[0])],
            &["".into(), "".into()],
        );
        assert!(result.unwrap_err().to_string().contains("cycle"));

        let dag_count: i64 = conn
            .query_row("SELECT COUNT(*) FROM task_dags", [], |r| r.get(0))
            .unwrap();
        assert_eq!(dag_count, 0, "a rejected DAG must not persist");
        assert!(
            agent::list_tasks(&conn).unwrap().is_empty(),
            "no task rows may exist for a rejected DAG"
        );
    ...
    async fn three_task_dag_executes_in_topological_order() {
        ...
        assert_eq!(
            ...
            "DAG must join the requirement's correlation chain"
        );
        ...
        let order_files: Vec<String> = loaded
            .execution_order
            .iter()
            .map(|id| file_of(id))
            .collect();
        ...
        for _ in 0..3 {
            ...
            assert_eq!(
                runnable.len(),
                1,
                "chain DAG must expose exactly one runnable task at a time"
            );
            let task = &runnable[0];
            let file = task.files.first().unwrap().clone();
            let result = agent::executor::apply_and_verify(&dir, &file, "old", "new content")
                .await
                .unwrap();
        ...
        assert_eq!(
            ...
            "execution must follow topological order"
        );
        assert!(
            agent::dag_runnable_tasks(&conn, &record.id)
                .unwrap()
                .is_empty(),
            "nothing left to run"
        );
    ...
    async fn failure_blocks_dependents_but_not_independent_tasks() {
        ...
        let result = agent::executor::apply_and_verify(&dir, "src/lib.rs", original_rs, broken)
            .await
            .unwrap();
        ...
        assert_eq!(
            blocked,
            vec![c_id.clone()],
            "exactly the dependent task gets blocked"
        );
        assert_eq!(
            agent::get_task(&conn, &c_id).unwrap().status,
            status::BLOCKED
        );
    ...
    async fn second_node_failure_rolls_back_only_itself_and_keeps_node_one() {
        ...
        let ok = agent::executor::apply_and_verify(&dir, "notes.md", "old notes", "new notes")
            .await
            .unwrap();
        ...
        assert!(
            fail.verification.contains("cargo check failed"),
            "got: {}",
            fail.verification
        );
        assert!(
            fail.verification.contains("error"),
            "compiler diagnostics must be present: {}",
            fail.verification
        );
        agent::update_status(
            &conn,
        ...
        assert_eq!(
            ...
            "new notes",
            "node 1's applied change must survive"
        );
        assert_eq!(
            agent::get_task(&conn, n1).unwrap().status,
            status::COMPLETED
        );
        ...
        assert_eq!(
            agent::get_task(&conn, n2).unwrap().status,
            status::ROLLED_BACK
        );
        ...
        assert!(agent::dag_runnable_tasks(&conn, &record.id)
            .unwrap()
            .is_empty());
        ...
        assert!(
            orphan.contains("nonexistent-dag"),
            "orphan error names the dag: {orphan}"
        );
        let premature = crate::governance::promotion::request_promotion(&conn, n3, None).unwrap();
        let refusal = crate::governance::promotion::apply_to_workspace(
            &conn,
            &premature.id,
            &dir,
            "readme.md",
        ...
        .to_string();
        assert!(
            refusal.contains("blocked") && refusal.contains("evidence gate"),
            "promotion refusal explains itself: {refusal}"
        );
    ...
    async fn retry_success_reopens_blocked_dependents_via_real_cargo_check() {
        ...
        agent::record_task_outcome_atomic(
            ...
            &fail.verification,
            Some("verification failed"),
            Some("original content restored"),
        )
        .unwrap();
        assert_eq!(
            agent::get_task(&conn, &notes_task).unwrap().status,
            status::BLOCKED,
            "dependent must block after the failure"
        );
        assert!(agent::dag_runnable_tasks(&conn, &record.id)
            .unwrap()
            .is_empty());
        ...
        assert_eq!(
            ...
            crate::intelligence::reliability::FailureClass::CompileError
        );
        ...
        assert_eq!(runnable.len(), 1);
        assert_eq!(
            runnable[0].id, retry_id,
            "the retry is the only runnable work"
        );
        assert_eq!(
            agent::get_task(&conn, &notes_task).unwrap().status,
            status::BLOCKED
        ...
        assert_eq!(
            agent::get_task(&conn, &notes_task).unwrap().status,
            status::PLANNING,
            "dependent must reopen after the retry succeeds"
        );
        let runnable = agent::dag_runnable_tasks(&conn, &record.id).unwrap();
        assert_eq!(runnable.len(), 1);
        ...
        let chain =
            crate::governance::ledger::list_by_correlation(&conn, &req.correlation_id).unwrap();
        assert!(chain.iter().any(|e| e.event_type == "task_retried"));
        assert!(chain
            .iter()
            .any(|e| e.event_type == "task_planned" && e.payload.contains("reopened")));
        assert_eq!(
            std::fs::read_to_string(dir.join("src").join("lib.rs")).unwrap(),
            fixed
        );
    ...
    async fn north_star_full_autonomous_loop_add_validation_with_test() {
        ...
        for (task_id, required) in [
            (&record.task_ids[0], "coding"),
            (&record.task_ids[1], "testing"),
        ] {
            let chosen =
                crate::intelligence::matcher::best_match(&profiles, &[required.to_string()])
                    .unwrap();
            assert!(
                chosen.missing.is_empty(),
                "the matched worker must actually cover '{required}'"
            );
        ...
        let assigned: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM agent_tasks WHERE dag_id = ?1 AND worker_id = 'coder'",
        ...
        let mut proposed: std::collections::HashMap<String, (&str, &str)> =
        ...
        for step in 0..2 {
            ...
            let premature = crate::governance::promotion::request_promotion(
                &conn,
                &task.id,
            ...
            assert_eq!(
                ...
                "no evidence yet -> not promotable"
            );
            ...
            assert!(
                !result.rolled_back,
                "step {step} must verify cleanly: {}",
                result.verification
            );
            assert!(
                result.verification.contains("cargo check passed"),
                "got: {}",
                result.verification
            );
            ...
            let verdict = crate::governance::promotion::request_promotion(
                &conn,
                &task.id,
                task.correlation_id.as_deref(),
            )
        ...
        assert_eq!(
            agent::dag_runnable_tasks(&conn, &record.id).unwrap().len(),
            0,
            "loop complete"
        );
        ...
        assert!(
            saved.contains("ErrorKind::InvalidInput"),
            "validation must be in the real file"
        );
        assert!(
            std::fs::read_to_string(dir.join("src").join("save_tests.rs"))
                .unwrap()
                .contains("empty_path_is_rejected")
        );

        // ---- Real `cargo test` passes against the modified code ----
        let (tests_passed, test_output) = crate::bootstrap::git::run_tests(&dir, "src/save.rs")
            .await
            .unwrap();
        ...
        assert!(
            test_output.contains("empty_path_is_rejected"),
            "the new validation test must have actually run"
        );
        ...
        assert_eq!(
            chain
                .iter()
                .filter(|e| e.event_type == "task_planned")
                .count(),
            2
        );
        ...
        assert!(
            verification.valid,
            "hash chain must verify after the full loop: {:?}",
    ...
    fn single_task_flow_is_unchanged_by_dag_support() {
        ...
        assert!(
            task.dag_id.is_none(),
            "single-task flow must not acquire a dag_id"
        );
        ...
        .unwrap();
        assert_eq!(
            agent::get_task(&conn, "solo-task").unwrap().status,
            status::COMPLETED
        );
