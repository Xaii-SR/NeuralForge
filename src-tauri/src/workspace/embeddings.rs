...
pub fn build_local_index(workspace_root: String) -> Result<usize, String> {
    ...
    std::fs::write(output_dir.join("chunks.json"), &json)
...
#[tauri::command]
pub fn query_codebase_semantic(
    query: String,
    max_results: usize,
    workspace_root: String,
) -> Result<Vec<QueryResult>, String> {
    let root = std::path::Path::new(&workspace_root);
    let raw = std::fs::read_to_string(root.join(".neuralforge/embeddings.json"))
        .map_err(|e| format!("read: {e}"))?;
    let chunks: Vec<VectorizedChunk> =
        serde_json::from_str(&raw).map_err(|e| format!("parse: {e}"))?;
    if chunks.is_empty() {
        return Ok(vec![]);
    }

    use fastembed::{EmbeddingModel, InitOptionsWithLength};
    let mut model = fastembed::TextEmbedding::try_new(InitOptionsWithLength::new(
    ...
    let mut scored: Vec<(f32, &VectorizedChunk)> = chunks
        .iter()
        .map(|c| (cosine_similarity(qv, &c.vector), c))
        .collect();
...
pub fn generate_local_embeddings(workspace_root: String) -> Result<usize, String> {
    ...
    let vectorized: Vec<VectorizedChunk> = chunks
        .into_iter()
        .zip(vectors)
        .map(|(c, v)| VectorizedChunk {
    ...
    std::fs::write(out_dir.join("embeddings.json"), &out_json)
