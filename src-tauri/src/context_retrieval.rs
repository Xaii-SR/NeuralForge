...
impl ContextRetrieval {
    ...
    pub fn retrieve_with_db(
        ...
        for sm in &symbol_matches {
            ...
            if seen_paths.contains(&sm.file_path) {
                continue;
            }
            ...
            if let Ok(content) = conn.query_row(
                ...
                rusqlite::params![sm.file_path, format!("%{}%", sm.name)],
                |r| r.get::<_, String>(0),
            ) {
                ranked.push(RankedFile {
                    ...
                    reason: format!(
                        "Symbol match: {} {} ({}:{})",
                        sm.kind, sm.name, sm.file_path, sm.start_line
                    ),
                    matched_symbols: vec![format!(
                        "{} {} @ line {}",
                        sm.kind, sm.qualified_name, sm.start_line
                    )],
        ...
        for result in &raw_results {
            ...
            let symbols: Vec<String> = symbol_matches
                .iter()
                .filter(|s| s.file_path == result.path)
            ...
            ranked.push(RankedFile {
                ...
                reason: if symbols.is_empty() {
                    ...
                    format!("{} symbol(s) matched", symbols.len())
                },
    ...
    pub fn find_definition(
        conn: &rusqlite::Connection,
        name: &str,
    ) -> Result<Option<SymbolMatch>, String> {
        Self::find_symbols(conn, name, 1).map(|mut v| {
            if v.is_empty() {
                None
            } else {
                Some(v.remove(0))
    ...
    pub fn find_references(
        ...
        if let Some(ep) = exclude_path {
            ...
            let rows = stmt
                .query_map(rusqlite::params![pattern, ep], |r| r.get::<_, String>(0))
            ...
            let mut stmt = conn
                .prepare("SELECT DISTINCT path FROM chunks WHERE content LIKE ?1 LIMIT 20")
                .map_err(|e| format!("ref query: {e}"))?;
            let rows = stmt
    ...
    pub fn find_implementations(
        ...
        let mut stmt = conn
            .prepare("SELECT DISTINCT path FROM chunks WHERE content LIKE ?1 LIMIT 10")
    ...
    pub fn plan_rename_refactor(
        ...
        let def = Self::find_definition(conn, symbol_name)?
            .ok_or(format!("Symbol '{}' not found", symbol_name))?;
        let refs = Self::find_references(conn, symbol_name, Some(&def.file_path))?;
        let mut steps = vec![format!(
            "Rename definition of `{}` in {}",
            symbol_name, def.file_path
        )];
        for file in &refs {
            steps.push(format!(
                "Update references to `{}` in {}",
                symbol_name, file
            ));
        }
        Ok(steps)
    ...
    fn rank_files(query: &str, scan: &ScanResult, max_files: usize) -> Vec<RankedFile> {
        ...
        for file in &scan.files {
            ...
            for w in &words {
                if rel.to_lowercase().contains(w) {
                    score = score.max(90);
                }
            }
            let hints: [(&str, &[&str]); 3] = [
            ...
            for (l, exts) in &hints {
                if exts.iter().any(|e| ql.contains(e)) && lang == *l {
                    score = score.max(85);
                }
            }
        ...
        scored
            .into_iter()
            .map(|(priority, file, language, symbols)| RankedFile {
    ...
    fn compute_priority(path: &str, query: &str, symbols: &[String]) -> u8 {
        ...
        if name.contains(&ql) {
            100
        } else if pl.contains(&ql) {
            90
        } else if symbols.iter().any(|s| s.to_lowercase().contains(&ql)) {
            80
        } else {
            50
        }
    ...
    fn build_reason(priority: u8, symbols: &[String]) -> String {
        match priority {
            100 => "Exact filename match".into(),
            90 => "Path component match".into(),
            85 => "Language match".into(),
            80 => format!(
                "Symbol match: {}",
                symbols.first().unwrap_or(&String::new())
            ),
            1 => "Low-relevance candidate".into(),
            _ => format!("Priority {}", priority),
        }
    }
}
...
mod tests {
    ...
    fn ranks_files_by_name_match() {
        ...
        fs::write(d.join("src/auth.rs"), "fn authenticate() {}").unwrap();
        fs::write(d.join("config.json"), "{}").unwrap();
        fs::write(d.join("main.ts"), "console.log('hi')").unwrap();
        let scan = workspace_scanner::scan_workspace(&d).unwrap();
    ...
    #[test]
    fn analyze_updates_context() {
        let d = td();
        fs::write(d.join("lib.rs"), "pub fn run() {}").unwrap();
        let mut ctx = AgentContext::new("t", "Improve", d.clone());
        let _ = analyze_workspace_for_agent(&mut ctx, "improve", None).unwrap();
        assert!(!ctx.relevant_files.is_empty());
        fs::remove_dir_all(&d).ok();
    }
    #[test]
    fn find_symbols_via_db() {
        let d = td();
        fs::write(
            d.join("lib.rs"),
        ...
        .unwrap();
        let conn = crate::database::open_for_workspace(&d).unwrap();
        crate::database::indexer::index_workspace(&conn, &d).unwrap();
        let symbols = ContextRetrieval::find_symbols(&conn, "compute", 10).unwrap();
        assert!(!symbols.is_empty());
        assert_eq!(symbols[0].kind, "function");
        let syms2 = ContextRetrieval::find_symbols(&conn, "User", 10).unwrap();
        assert!(!syms2.is_empty());
        assert_eq!(syms2[0].kind, "struct");
        drop(conn);
        fs::remove_dir_all(&d).ok();
    }
    #[test]
    fn find_definition_and_references() {
        let d = td();
        fs::write(d.join("lib.rs"), "pub fn authenticate()->bool{true}\n").unwrap();
        fs::write(d.join("main.rs"), "mod lib;\nfn main(){authenticate();}\n").unwrap();
        let conn = crate::database::open_for_workspace(&d).unwrap();
        crate::database::indexer::index_workspace(&conn, &d).unwrap();
        let def = ContextRetrieval::find_definition(&conn, "authenticate").unwrap();
        assert!(def.is_some());
        assert_eq!(def.unwrap().file_path, "lib.rs");
        let refs =
            ContextRetrieval::find_references(&conn, "authenticate", Some("lib.rs")).unwrap();
        assert!(refs.contains(&"main.rs".to_string()));
        drop(conn);
        fs::remove_dir_all(&d).ok();
    }
    #[test]
    fn rename_refactor_plan() {
        let d = td();
        fs::write(d.join("auth.rs"), "pub fn login()->bool{true}\n").unwrap();
        fs::write(d.join("main.rs"), "mod auth;\nfn main(){auth::login();}\n").unwrap();
        let conn = crate::database::open_for_workspace(&d).unwrap();
        crate::database::indexer::index_workspace(&conn, &d).unwrap();
        let plan = ContextRetrieval::plan_rename_refactor(&conn, "login").unwrap();
        assert!(plan.len() >= 2);
        assert!(plan[0].contains("auth.rs"));
        assert!(plan[1].contains("main.rs"));
        drop(conn);
        fs::remove_dir_all(&d).ok();
    }
}
