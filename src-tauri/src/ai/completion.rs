...
pub fn reset_debounce() {
    LAST_REQUEST_MS.store(0, Ordering::Relaxed);
}
...
pub fn check_prediction_cache(file_path: &str, new_suffix: &str) -> Option<(String, bool, String)> {
    ...
    if entry.suffix.starts_with(new_suffix) && new_suffix.len() < entry.suffix.len() {
        ...
        return Some((
            remaining.to_string(),
            true,
            entry.suffix[consumed..].to_string(),
        ));
    }
    None
}
...
pub struct PredictionResponse {
    ...
    pub fim_prompt: String,
    pub cached: bool,
    pub completion: String,
    pub remaining_suffix: String,
}
#[derive(Serialize, Clone, Debug)]
pub struct GhostTextStreamPayload {
    pub token: String,
    pub done: bool,
...
pub fn extract_prediction_window(
    ...
    let ss = (cursor + 1).min(total);
    let se = (ss + SL).min(total);
    let suffix: String = if ss < se {
        lines[ss..se].join("\n")
    } else {
        String::new()
    };
...
pub fn run_prediction_pipeline(
    file_path: String,
    content: String,
    cursor_line: usize,
    cursor_column: usize,
...
pub async fn stream_inline_diff(
    ...
    for (i, orig_line) in orig_lines.iter().enumerate() {
        let matched = gen_lines
            .iter()
            .enumerate()
            .skip(i)
            .find(|(_, gl)| *gl == orig_line);
        if let Some((j, _)) = matched {
            gen_used[j] = true;
            diff_ops.push(DiffLinePayload {
                op: DiffOp::Equal,
                line: orig_line.to_string(),
                line_index: i,
    ...
    for (j, gen_line) in gen_lines.iter().enumerate() {
        if !gen_used[j] {
            diff_ops.push(DiffLinePayload {
                op: DiffOp::Insert,
                line: gen_line.to_string(),
                line_index: orig_lines.len() + j,
                request_id: request_id.clone(),
            });
        }
    }
...
/// caller.
async fn call_ollama_fim(
    health: &HealthRegistry,
    providers: &[ProviderConfig],
    fim_prompt: &str,
) -> Result<String, String> {
    provider_router::complete_fim(providers, health, fim_prompt, 64, 0.1)
        .await
...
#[tauri::command]
pub fn get_prediction_with_fim(
    file_path: String,
    content: String,
    cursor_line: usize,
    cursor_column: usize,
    ...
    let t = match template.to_lowercase().as_str() {
        "codellama" => FimTemplate::CodeLlama,
        _ => FimTemplate::StarCoder,
    };
    run_prediction_pipeline(file_path, content, cursor_line, cursor_column, t)
}
#[tauri::command]
pub fn store_prediction_result(file_path: String, suffix: String, completion: String) {
    cache_prediction(&file_path, &suffix, &completion);
}
#[tauri::command]
...
mod tests {
    ...
    fn extract_window_basic() {
        let c = (0..200)
            ...
            .collect::<Vec<_>>()
            .join("\n");
        let x = extract_prediction_window("t.rs".into(), c, 100, 5);
        assert_eq!(x.prefix.lines().count(), 50);
        assert_eq!(x.suffix.lines().count(), 20);
    }
    #[test]
    fn extract_window_start() {
        let x = extract_prediction_window("t.rs".into(), "a\nb".into(), 0, 0);
        assert_eq!(x.prefix.lines().count(), 0);
    }
    #[test]
    fn extract_window_end() {
        let x = extract_prediction_window("t.rs".into(), "a\nb".into(), 1, 0);
        assert_eq!(x.suffix.lines().count(), 0);
    }
    #[test]
    fn extract_window_empty() {
        let x = extract_prediction_window("e.rs".into(), String::new(), 0, 0);
        assert_eq!(x.prefix, "");
    }
    #[test]
    fn fim_starcoder() {
        let p = FimPrompt {
            prefix: "fn a(".into(),
            suffix: " -> i32 {}".into(),
            template: FimTemplate::StarCoder,
        };
        let f = p.format();
        assert!(f.contains("<fim-prefix>"));
        assert!(f.contains("<fim-suffix>"));
    }
    #[test]
    fn fim_codellama() {
        let p = FimPrompt {
            prefix: "def a():".into(),
            suffix: "  pass".into(),
            template: FimTemplate::CodeLlama,
        };
        let f = p.format();
    ...
    #[test]
    fn cache_miss_first() {
        PREDICTION_CACHE.lock().unwrap().take();
        assert!(check_prediction_cache("t.rs", "x").is_none());
    }
    #[test]
    fn cache_hit() {
        cache_prediction("t.rs", "abcdef", "ghijkl");
        let r = check_prediction_cache("t.rs", "abc").unwrap();
        assert!(r.1);
        assert_eq!(r.0, "jkl");
    }
    #[test]
    fn cache_miss_diff_file() {
        cache_prediction("o.rs", "abc", "xyz");
        assert!(check_prediction_cache("t.rs", "abc").is_none());
    }
    #[test]
    fn pipeline_miss() {
        PREDICTION_CACHE.lock().unwrap().take();
        let r = run_prediction_pipeline(
            "t.rs".into(),
            "a\nb\nc".into(),
            1,
            0,
        ...
        assert!(!r.cached);
        assert!(!r.fim_prompt.is_empty());
    }
    #[test]
    fn pipeline_hit() {
        cache_prediction("t.rs", "c", "ompletion");
        let r = run_prediction_pipeline(
            "t.rs".into(),
            "a\nb\nc".into(),
            2,
        ...
        assert!(r.cached);
        assert_eq!(r.completion, "ompletion");
    }
    #[test]
    fn debounce_pass() {
        reset_debounce();
