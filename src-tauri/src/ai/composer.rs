...
pub struct AgentResponse {
    pub text: Vec<ComposerMessage>,
    pub autonomous_sources: Vec<SourceInfo>,
}
...
pub async fn execute_composer_command_stream(
    ...
    let co = child.stdout.take();
    let ce = child.stderr.take();
    {
        let mut c = state.children.lock().map_err(|e| e.to_string())?;
        c.insert(block_id.clone(), child);
    }
    if let Some(stdout) = co {
        use std::io::BufRead;
...
fn run_agentic_loop(
    ...
    for _depth in 0..MAX_RECURSION_DEPTH {
        ...
        if let Some(caps) = RE_SEARCH.captures(&response) {
            ...
            let _ = app.emit(
                "agent-status",
                AgentStatusPayload {
                    message: format!("Searching codebase for '{}'...", query),
                },
            );
    ...
    if final_text.is_empty() {
        final_text = "[Max recursion depth reached]".to_string();
    }
...
fn search_codebase_with_sources(query: &str, workspace_root: &str) -> (String, Vec<SourceInfo>) {
    ...
    let chunks: Vec<crate::workspace::embeddings::VectorizedChunk> =
    ...
    let mut model = match fastembed::TextEmbedding::try_new(fastembed::InitOptionsWithLength::new(
        fastembed::EmbeddingModel::AllMiniLML6V2,
    )) {
        Ok(m) => m,
        Err(_) => return (String::new(), vec![]),
    };
    let qv = match model.embed(vec![query], None) {
        Ok(v) => v,
        Err(_) => return (String::new(), vec![]),
    };
    let qv = &qv[0];

    let mut scored: Vec<(f32, &crate::workspace::embeddings::VectorizedChunk)> = chunks
        .iter()
        .map(|c| {
            (
                crate::workspace::embeddings::cosine_similarity(qv, &c.vector),
                c,
            )
        })
        .collect();
    ...
    let sources: Vec<SourceInfo> = scored
        .iter()
        .map(|(s, c)| SourceInfo {
    ...
    let txt = scored
        .into_iter()
        .map(|(s, c)| format!("File: {} (score: {:.2})\n{}", c.file_path, s, c.text))
        .collect::<Vec<_>>()
        .join("\n\n---\n\n");
    (txt, sources)
}
...
pub fn send_composer_message(
    ...
    let session = sessions
        ...
        .find(|s| s.session_id == session_id)
        .ok_or_else(|| format!("Session {} not found", session_id))?;
    let (final_text, auto_sources) =
        run_agentic_loop(&app, ".", session, &content, semantic_context.as_deref());
    ...
    Ok(AgentResponse {
        text: session.message_history.clone(),
        autonomous_sources: auto_sources,
    })
}
...
mod tests {
    ...
    fn empty_returns_none() {
        let (t, b) = parse_code_blocks("text.");
        assert_eq!(t, "text.");
        assert!(b.is_empty());
    }
    #[test]
    fn prompt_includes_file() {
        let d = std::env::temp_dir().join("c_test");
        std::fs::create_dir_all(&d).unwrap();
        let f = d.join("t.rs");
        std::fs::write(&f, "fn e() {}").unwrap();
        let files = vec![f.to_string_lossy().to_string()];
        let p = build_system_prompt(&files);
        assert!(p.contains("t.rs"));
        assert!(p.contains("e()"));
        std::fs::remove_dir_all(&d).ok();
    }
    #[test]
    fn prompt_includes_tool_instructions() {
