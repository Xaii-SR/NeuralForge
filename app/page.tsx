...
export default function Home() {
  ...
  const [bottomTab, setBottomTab] = useState<"terminal" | "logs" | "agent" | "workbench" | "extensions" | "bootstrap" | "governance" | "workers">("terminal");
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div style={{ height: "var(--nf-bottom-h, 288px)" }} className="flex shrink-0 flex-col border-t border-neutral-200 dark:border-neutral-800">
            <div className="flex h-9 shrink-0 gap-1 border-b border-neutral-200 bg-neutral-50 px-2 dark:border-neutral-800 dark:bg-neutral-900">
              {(["terminal","logs","agent","workbench","extensions","bootstrap","governance","workers"] as const).map((t) => (
                <button key={t} onClick={() => setBottomTab(t)} className={`${TAB_BUTTON} ${bottomTab === t ? TAB_ACTIVE : TAB_INACTIVE}`}>{t === "terminal" ? "Terminal" : t === "logs" ? "Logs" : t === "agent" ? "Agent" : t === "workbench" ? "Workbench" : t === "extensions" ? "Extensions" : t === "bootstrap" ? "Bootstrap" : t === "governance" ? "Governance" : "Workers"}</button>
              ))}
            </div>
